 ...
 */
class ServiceWrapperException extends ContextAwareException {
    public function __construct(string $serviceName, array $context = [], \Throwable $previous = null) {
        parent::__construct(
            ErrorCodes::SERVICE_WRAPPER_ERROR,
            sprintf('Service wrapper error for "%s".', $serviceName),
            array_merge(['service' => $serviceName], $context),
            ErrorCodes::getSeverity(ErrorCodes::SERVICE_WRAPPER_ERROR),
            0,
            $previous
        );
    }
